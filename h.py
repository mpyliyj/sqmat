...
import spear3
spear3.ring.getMatLine()

# --- calculate chromaticity response matrix
chsext = [spear3.ring.getElements('sext','SDI')[0],spear3.ring.getElements('sext','SFI')[0]]
spear3.ring.getChrm(chsext)
...
L = [spear3.ring.bl[k].L for k in spear3.ring.klist]

tol = 1e-12
betax0,betay0 = spear3.ring.betax[0],spear3.ring.betay[0]
alphax0,alphay0 = spear3.ring.alfax[0],spear3.ring.alfay[0]
phix0,phiy0 = spear3.ring.nux*np.pi*2,spear3.ring.nuy*np.pi*2
...
x0list = [3.0e-3, 9.0e-3,15.0e-3]
y0list = [1.0e-3, 3.0e-3, 4.0e-3]
xylist = []
...
def cirDist(k2s):
	...
	norder = 7
        ...
        # slow but general ---
	sext = spear3.ring.getElements('sext','SD2MS')[0]
        sext.put('K2',k2s[0])
	sext = spear3.ring.getElements('sext','SF2MS')[0]
        sext.put('K2',k2s[1])
        sext = spear3.ring.getElements('sext','SD3MS')[0]
        sext.put('K2',k2s[2])
        sext = spear3.ring.getElements('sext','SF3MS')[0]
        sext.put('K2',k2s[3])
        sext = spear3.ring.getElements('sext','SD4MS')[0]
        sext.put('K2',k2s[4])
        sext = spear3.ring.getElements('sext','SF4MS')[0]
        sext.put('K2',k2s[5])
        sext = spear3.ring.getElements('sext','SD5MS')[0]
        sext.put('K2',k2s[6])
        sext = spear3.ring.getElements('sext','SF5MS')[0]
        sext.put('K2',k2s[7])

        spear3.ring.cchrom1(chsext,[2.,2.])

        K2 = [spear3.ring.bl[k].K2 for k in spear3.ring.klist]
        
        temp = [CTPS(0,i) for i in xrange(1,nv+1)]
        for i,k in enumerate(spear3.ring.klist):
            temp = sm.lmPass(spear3.ring.mlist[i][:nv,:nv],temp)
            temp = sm.thickSextPass(L[i],K2[i],1,temp)
        temp = sm.lmPass(spear3.ring.mlist[-1][:nv,:nv],temp)
	...
	xlist = np.linspace(1e-6, 15e-3,25) # a list of x
	ylist = np.linspace(1e-6,  5e-3,10) # a list of y
