...
def mpi_controller(task_list):
    ...
    print '%4i received out of %4i'%(nrecv,njobs)
    return result
...
def mpi_worker(f,nvar,nobj):
    ...
    while True:
        ...
        print('slave has %4i done and sent'%status.tag)
...
def mpi_run():
    ...
    if rank == 0:
        ...
        fid = h5py.File('./data/sls_20161122_5.h5','r')
        last = fid.keys()[-1]
    ...
    for ng in range(ngen):
        if rank == 0:
            ...
            #--- save results
            fid = h5py.File('./data/sls_20161122_6.h5','a')
            fid['%04i'%(ng+1)] = pop
            fid.close()
