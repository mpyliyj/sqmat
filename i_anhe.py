...
import anhe_9 as anhe
anhe.ring.getMatLine()

# --- calculate chromaticity response matrix
#chsext = anhe.ring.getElements('sext')[::2][-2:]
#anhe.ring.getChrm(chsext)
...
L = [anhe.ring.bl[k].L for k in anhe.ring.klist]
...
def cirDist(k2s):
	...
	sext = anhe.ring.getElements('sext','sl1')[0]
        sext.put('K2',k2s[5])
	sext = anhe.ring.getElements('sext','sh1w')[0]
        sext.put('K2',k2s[0]+k2s[6])
	sext = anhe.ring.getElements('sext','sh3w')[0]
        sext.put('K2',k2s[1]+k2s[7])
	sext = anhe.ring.getElements('sext','sh4w')[0]
        sext.put('K2',k2s[2]+k2s[8])

        K2 = [anhe.ring.bl[k].K2 for k in anhe.ring.klist]
        
        temp = [CTPS(0,i) for i in xrange(1,nv+1)]
        for i,k in enumerate(anhe.ring.klist):
            temp = sm.lmPass(anhe.ring.mlist[i][:nv,:nv],temp)
            temp = sm.thickSextPass(L[i],K2[i],4,temp)
        temp = sm.lmPass(anhe.ring.mlist[-1][:nv,:nv],temp)
