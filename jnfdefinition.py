...
def initialize():
	...
	if display=="initialize": print "in initialize 1: zeros=", zeros, " len(diagonal)=", len(diagonal) , " tol=",tol
	#print diagonal
	tmp=np.zeros(len(mp))
	for j in range(0,len(zeros)): tmp[zeros[j]]=j
	...
	if display=="initialize": print "in initialize 2: zeros=", zeros
...
def solvingNonzeroRows():
	...
	if display=="solvingNonzeroRows": print "At p0, in non-zero Solving non-zero row, j=", j, " eigenposition=", eigenposition[j] 
	if display=="solvingNonzeroRows": print "in solvingNonzeroRows, At p0, eik= ", eik, " j=",j ," zeros[eik-1]=", zeros[eik-1]
	if eik==0.0 or (eik>0 and j>zeros[eik-1]):
		...
		if display=="solvingNonzeroRows":print "At p1, in solvingNonzeroRows, eik= ", eik, " j=",j ," zeros[eik-1]=", zeros[eik-1]
		if display=="solvingNonzeroRows":print "At p1, np.dot(mp[j][j+1:],x[j+1:])=", np.dot(mp[j][j+1:],x[j+1:]), " mp[j][j]=", mp[j][j]
		#print "mp[j]=", mp[j]
		if display=="solvingNonzeroRows":print "x=", x
		x[j]=-np.dot(mp[j][j+1:],x[j+1:])/mp[j][j]
		uu=-np.dot(mp[j][j+1:],x[j+1:])/mp[j][j]
		yy=x[j]
		if display=="solvingNonzeroRows": print  "At p1, Found x[", j, "]=", x[j], "check uu.yy.", uu,yy
		if display=="solvingNonzeroRows":  print  "At p1, Found x[", j, "]=", x[j]
	elif j < zeros[eik-1]:
		#For other non-zero rows, the RHS of the equation is determined by the known eigenvectors with lower indexes
		if display=="solvingNonzeroRows": print "At p2, in non-zero", " Y[[j]]=", np.dot(mp[j][j+1:],x[j+1:]), " eik=", eik, " j=", j, " x[j]=", x[j]
		if display=="solvingNonzeroRows": print "At p2, Sum[y[eik,m-1]Xi[[m-1,j]],{m,eik,2,-1}]=",sum( [tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1) ])    		
		x[j]=(sum( [tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1) ])-np.dot(mp[j][j+1:],x[j+1:]))/mp[j][j]    		
		if display=="solvingNonzeroRows": print "At p2, Found x[", j, "]=", x[j]
		if display=="solvingNonzeroRows": print "At p2,  Y=", np.dot(mp[j][j:],x[j:]), ",sum( [tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1) ])",sum( [tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1) ])," X=", x
	return

def solvingZeroRows():
	global eik,mp,j,x,tm,eigenposition,zeros,xi,diagonal
	#For first eigenvector, or the last sequential rows of non-zero rows in the matrix, the RHS of the equation is 0.
	if display=="solvingZeroRows": print "p0 in solvingZeroRows, j=", j, " eigenposition=", eigenposition[j] 
	if display=="solvingZeroRows":print "in solvingZeroRows, eik= ", eik, " j=",j ," zeros[eik-1]=", zeros[eik-1]
	if display=="solvingZeroRows":print "np.dot(mp[j][j+1:],x[j+1:])=", np.dot(mp[j][j+1:],x[j+1:]), " mp[j][j]=", mp[j][j]
	#print "mp[j]=", mp[j]
	if display=="solvingZeroRows1":print "x=    ", x
	if display=="solvingZeroRows":print "zeros=    ", zeros
	#ep is the index of eigenvalue at position j
	ep=eigenposition[j][1]	
	vi=[ i for i in range(0,j+1) if abs(mp[j][i])>tol ]
	if display=="solvingZeroRows": print "vi=",vi, " mp[j]=",[ mp[j][i] for i in range(0,j+1) ] 
	if vi==[]: 
		if display=="solvingZeroRows": print "p1", "Y[[j]]=", " eik=", eik, " j=", j, " vi=", vi
		if display=="solvingZeroRows": print tm[eik,0],xi[0][j] 
		if display=="solvingZeroRows": print " \n y[", eik, ",", ep, "]=",tm[eik,ep],", Y[[", j, "]]= ",np.dot(mp[j][j+1:],x[j+1:]),  " -Sum[y[eik,m-1]Xi[[m-1,j]],{m,eik,2,-1}]=", sum([tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1)])
		tm[eik,ep]=np.dot(mp[j][j+1:],x[j+1:])-sum([tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1)])
		if display=="solvingZeroRows": print " \n after asign y[eik,ep], y[", eik, ",", ep, "]=",tm[eik,ep],", Y[", j, "]= ", np.dot(mp[j][j+1:],x[j+1:]), ", Sum[y[eik,m-1]Xi[[m-1,j]],{m,eik,2,-1}]=", sum([tm[eik,m-1]*xi[m-1][j] for m in np.arange(eik,0,-1)])
	return
...
def eigenv():
	...
	if display=="eigenv": print "in eigen, len(mp)=", len(mp)
	zeros,eigenposition,diagonal,tm=initialize()
	if display=="eigenv": print "in eigenv(): zeros=",zeros
	if display=="eigenv": print "in eigen, pos2", len(diagonal)
	xi=[]
	...
	while eik<len(zeros):
		x=initializeX()
		j=zeros[eik]-1
		if display=="eigenv": print "\n  \n   Solve: eik= ", eik, " \n \nY = ", " X= ", x

		if display=="eigenv": print "xi=", xi, ", j=",j


		while j>=0:
			#Solve equations for each row of the eigenvector # eik, start from the last row at position eik - 1 to 1
			if display=="eigenv": print "\n \n p0 Solve j=", j, " eigenposition[[j]]= ",eigenposition[j], "Y="
			if abs(eigenposition[j][0])<tol:
				solvingZeroRows()
			else:
				solvingNonzeroRows()
				if display=="eigenv":print "in solvingNonzeroRows,now j=",j
			if display=="eigenv":print "now j=", j
			if display=="eigenv":print "now eik=", eik, " x=",x
			j=j-1
...
def prp(a,m):
	#print complex array
	for j in range(m):
    		print '  %10.1f %6s  '%(a[0][j].real,a[1][j]),
		if (j-4)%5==0: print ""
	print ""
...
def rkchop(m):
	...
	ln=len(m)
   	while rank > 0:
		...
		if display=='rkchop': print "len(chp)=",len(chp), "maxmp=",maxmp
		if maxmp<tol:
			#print "last mp=", prm(mp*1e6,10,10)
			return array(rkm)
		mp=mp/maxmp    #when mp is too small, we need renormalize mp
		if display=='rkchop': prm(mp*1e6,10,10)
		mp[abs(mp)<tol]=0     #chop every term less than tol
		chp.append(mp)
    		rank = rk(chp[-1])
    		rkm.append(rank)
	#output is the rank of m, m.m, m.m.m, ...till the rank is 0
	if display=='rkchop': sv('junk1',chp)
   	return array(rkm)
...
def couplingelmentposition(tm): #tm is the eigenspace matrix in Xi.M=tm.Xi
    ...
    if abs(EigenspaceDimension - NumberofChains*(NumberofChains + 1)/2)>1e-4: sys.exit(0)
    ...
    CouplingElementsPosition = np.zeros([EigenspaceDimension])
    for i in range(len(T)):
          for j in range(T[i][1][1]-T[i][1][0]+1):
                CouplingElementsPosition[ T[i][1][0] + j ] = T[i][0][0] + j
...
def nullblocksvd(s):
    ...
    ln=len(s)
    ml=ln-rkchop(s)
    if display=="nullblocksvd": print "in nullblocksvd, ml=", ml
    ...
    if display=="nullblocksvd": sv('junk1',[ml,s,s0])
    return v0h,s0,v0
...
def gausselim(T):
    ...
    j=ln 
    couplingelm=ln
    couplingelmposition=np.zeros([ln])
    while couplingelm > 1:
        j = j - 1
	...
	couplingelmposition[j]=couplingelm
        ...
        v[couplingelm, couplingelm] = 1.0+0.j
        u = np.dot(v,u) 
        um = np.dot(um,vm)
        B = np.dot(v,np.dot(B,vm))

    if display=="gausselim": sv('junk1',B)
    if display=="gausselim": print "in gausselim, coupling element row position =",couplingelmposition 
    ml,couplingelmpositionTheory=couplingelmentposition(T)
    if display=="gausselim": print "max(|couplingelmposition-theoty|)=",max(abs(couplingelmposition-couplingelmpositionTheory))
    if display=="gausselim": print "couplingelmpositiontheoty=",couplingelmpositionTheory
    couplingelement=[ T[couplingelmposition[i]][i] for i in range(len(couplingelmposition)) if couplingelmposition[i]>0]
    if display=="gausselim": print "couplingelement=",abs(array(couplingelement))
    return u, np.dot(u,np.dot(T,um)), um

def gausselimacc(T):
    u1,J1,u1m=gausselim(T)
    u2,J,u2m=gausselim(J1)  #iterate gauss elimination once to make elimination of each upper column much more accuraate
    u=np.dot(u2,u1)
    um=np.dot(u1m,u2m)
...
def findchains(J):
    ...
    while chainremain != []:
        ...
        while it > 0:
		it = chainlinks[it-1] 
		if it>0: chain.append(it)

        chains.append(chain[::-1])
        chainremain = [ i for i in range(1,ln+1) if i not in [ j for subc in chains for j in subc] ]
    return chains[::-1]
...
def semiJ(J):
    ...
    X = np.identity(ln)*(1.0+0.0j)   #X serves as a set of initial transformation basis 
    fch=[i-1 for subc in ch for i in subc] #fch is the index sequence of the chain arrange according to Jordan chain
...
def jnfinsubspace(s):
    ...
    v1h,s1,v1 = nullblocksvd(s) #build columns of null space of powers of mn by iteration
    if display=="jnfinsubspace":
    ...
    U,T,Uh=triangularsubblocks(s1) #using QR decomposition to make avery super diagonal blocks upper triagular
    if display=="jnfinsubspace":
    ...
    u,pJ,um,u2,u1,u1m,u2m=gausselimacc(T)  #use iterated gauss elimination to obtain accurate non-normalized Jordan form
    if display=="jnfinsubspace":
    ...
    zm,sJ,z = semiJ(pJ) #Find chains from the list of general eigenvectors in pJ and sequence them in order to form semi-Jordan form where one vector transform into next after acted upon by the matrix mn from right only different by one facctor. 
    V = np.dot(zm,V)
    Vm =np.dot(Vm,z) 
    if display=="jnfinsubspace":
...
def semileftjordanbasis(mn):
	tm,xi=eigensubspace(mn) #Find a set of generalized eigenvectors to form an eigenspace for eigenvalue 0.
	if display=="semileftjordanbasis": prm(mn,10,10)
	if display=="semileftjordanbasis": print "tm=",tm
	if display=="semileftjordanbasis": print "tol=",tol	
	S,J,Sm,S2,S1,S1m,S2m=jnfinsubspace(tm) #From one eigensubspace build the semi-Jorda basis for semi-Jordan form J
...
def vJlnJM(J,mu):
	...
	v,JlnJM,normalizer,A=orthonormalize(v,JlnJM) #normalize all vectors in the basis
...
def normalize(u,J):
	...
	A=np.identity(len(u))*(1.+0j)	
	for i in range(len(u)):A[i,i]=1/norm[i]
	Am=np.linalg.inv(A)
	un=np.dot(A,u)
	...
	for link in chainposition:  
		for j in range(1,len(link)-1): #for every multiplier, the norm of all vectors down the chain from it must be increased by the multiplier in order to change to standard Jordan form.
			for k in range(j+1,len(link)):
				normalfactor[link[k]]=normalfactor[link[k]]*chain[link[j]]
	for link in chainposition:
		normalfactor[link[0]]=1.

	#3. Generate the matrix "nomalizer" for transformation from un to unn
	normalizer=np.identity(len(J))*1.+0j
...
def orthonormalize(us,Js):
	...
	u,J,normalizer,chain,A1,A1m=normalize(us,Js) #normalized semi-Jordan form J and normalized left basis u. So u=A.us, J=A1.Js.A1m
	maxchainlenposition, maxchainlen, chain, chainposition=findchainposition(J)
	t=np.identity(len(J))*(1+0.j)
	for link in chainposition: #for each jordan block, follow proceedure in "Gauge transform of JNF". Section: "How to make u[1] orthogonal to other eigenvectors."
		if len(link)==1: continue  #if len(link)=0, this is an single element eigenstate, so there is no other elements to be orthogonalized.
		subchain=[chain[i] for i in link] #the list a1,a2,a3,.. in semi-Jordan form J

		alp=[1+0.j] #alp=[a1,a1 a2, a1 a2 a3, a1 a2 a3 a4, a1 a2 a3 a4 a5]. a1, a2,.. are chain ratio.
		i=1
		while i <len(subchain):
			alp.append(alp[-1]*subchain[i])
			i=i+1
		alp=alp[1:] #drop first one alp=[a1,a1 a2, a1 a2 a3, a1 a2 a3 a4, a1 a2 a3 a4 a5]. a1, a2,.. are chain ratio.
		...
		beta=abeta/alp  #beta=X/alp is the solution for coefficients of gauge transform
		...
		abeta=np.insert(abeta,0,0) #this makes it easy to use indexes to identify which raio is used later
		for i in range(1,len(link)): #assing value to superdiagonal i, start from row=0 and column=i 
			ax=abeta[i]
			t[link[0], link[i]]=ax
			#print "link[0]=",link[0]," link[i]=",link[i]," ax=",ax
			j=1
			while i+j<len(link): #assigning value to super diagonal i, while j is the index along the superdiagonal 
	...
	uo,Jo,normalizer,chain,A2,A2m=normalize(unn,J) #So uo=A2.unn=A2.t.A1.us, Jo=A2.J.A2m=A2.A1.Js.A1m.A2m and normalizer.uo is standard Jordan basis
	A=d3(A2,t,A1)
...
def crossblockorthonormalize(u,J):
	...
	ene1=np.empty(len(v))*(1.+0.j) #column <e_i|e_0>
	...
	for k in range(1, len(chainposition)): #for all chain except the main chain construct the upper-triangular part of the transformation matrix
		...
		for i in range(len(link)):  #assing value to superdiagonal i, start from row=0 and column=i 
			ax=abeta[link[i]]
			t[link0[0], link[i]]=ax
			j=1
	...
	#See rule2 in "JordanNormalFormAndTriangularMatrix", section "How to make all vectors orthogonal to one main vectors while mainaining Jordan form invariant  "
	for i in range(1,len(link0)): #assing value to superdiagonal i, start from row=0 and column=i 
		ax=abeta[link0[i]]
		t[link0[0], link0[i]]=ax
		j=1
		while i+j<len(link0): #assigning value to super diagonal i, while j is the index along the superdiagonal 
	...
	u,J,normalizer,chain,A1,A1m=normalize(un,J)  #So u=A1.un=A1.t.u, J=A1.J.A1m, normalizer.u gives standard Jordan basis, so noramlizer.J.normalizer^(-1) is the standard Jordan form.
...
def UMexpJold(M, mu):
	...
	un,Jn=standardizeJNF(us,Js,unormalizer) #un, Jn is in the standard Jordan form, un=unormalizer.u
	...
	u1=np.dot(v,un) #u1=v.un is the left basis of ln(exp(-j*mu)M):   u1.ln(exp(-j*mu)M)=JlnJM.u1   (because JlnJM=v.un.ln(exp(-j*mu)M).unm.vm, so JlnJM.v.un=v.un.ln(exp(-j*mu)M)   )
	u2,J2,normalizer,A1=orthonormalize(u1,JlnJM) #Hence u2=A1.u1 is still the left basis of ln(exp(-j*mu)M):   u2.ln(exp(-j*mu)M)=J2.u2, and all the vectors in u2 are normalized
	#checkcorrelation(u2) #u2 have all vectors nomralized and each block has its first vector orthogonal to all others
	u3,J3,u3normalizer,A2=crossblockorthonormalize(u2,J2) #Hence u3=A2.u2 is still the left basis of ln(exp(-j*mu)M):   u3.ln(exp(-j*mu)M)=J3.u3, and all the vectors in u are normalized and all are orthogonal to u[0] even between Jordan blocks
	...
	U,J=standardizeJNF(u3,J3,u3normalizer) #U=u3normalizer.u3=u3normalizer.A2.A1.u1=u3normalizer.A2.A1.v.un=u3normalizer.A2.A1.v.unormalizer.u
...
def orderdotnew(u1,u2,order,powerindex,ypowerorder):
	...
	v1=array([ u1[i] for i in range(len(u1)) if sum(powerindex[i])==order and sum(powerindex[i][2:4])<=ypowerorder])
	v2=array([ u2[i] for i in range(len(u2)) if sum(powerindex[i])==order and sum(powerindex[i][2:4])<=ypowerorder])
	u1u2=np.dot(v1,v2)
	return u1u2
...
def findorder(i,chainposition):
	...
	chainindex,indexinchain=next(([k,sublist] for k, sublist in enumerate(chainposition) if i in sublist), -1) #Find which chain the row i is in and find the position in this chain
...
def findordernew(i,chainposition,u,powerindex,ypowerorder,u0idx):
	...
	a= [sum(powerindex[k])  for k,b in enumerate(abs(u[i])) if b/abs(u[0][u0idx])>1e-7 and sum(powerindex[k][2:4])<=ypowerorder]
	#print a,ypowerorder
	if a==[]:
...
def correlationatorder(u,chainposition,powerindex):
	mcc=np.tile(0j,[len(u)-1,len(u)-1])
	for i in range(1,len(u)):
		order=findorder(i,chainposition)
		for j in range(1,len(u)):
			mcc[i-1,j-1]=orderdot(ht(u[i]),u[j], order,powerindex)
	#if display==True: prm(mcc*1e2,len(mcc),len(mcc))
	return mcc	
...
def correlationatordernew(u,chainposition,powerindex,ypowerorder,u0idx):
	...
	for i in range(1,len(u)):
		order=findordernew(i,chainposition,u,powerindex,ypowerorder,u0idx) #order is determined by the row number i in u[i]
		for j in range(1,len(u)):
...
def correlationatorderall(u,chainposition,powerindex,ypowerorder,u0idx):
	...
	if ypowerorder==0:
		for i in range(len(u)):
			if display=="correlationatorderall": "print i=",i
			order=findordernew(i,chainposition,u,powerindex,u0idx) #order is determined by the row number i in u[i]
			if display=="correlationatorderall": "print i,order=",i,order
		...
		for i in range(len(u)):
			...
			for j in range(len(u)):
				mcc[i,j]=orderdot(ht(u[i]),u[j], order,powerindex)
	
	#if display==True: prm(mcc*1e2,len(mcc),len(mcc))
	return mcc	

def minimizeu0byorder(u,J,chainposition, powerindex):
	...
	maxchainlenposition, maxchainlen, chain, chainposition=findchainposition(J)
	#Construct the matrix equation for the transformation coefficients abeta
	mc=correlationatorder(u,chainposition, powerindex)
	ene0=np.empty(len(mc))*(1.+0.j) #column <e_i|e_0>
	for i in range(1,len(u)):
		order=findorder(i,chainposition) #For each i select the power order for the minimization
		ene0[i-1]=orderdot(ht(u[i]),u[0], order, powerindex)

	abeta=np.linalg.solve(mc,-ene0) #solve mc.X=-ene1
	abeta=np.insert(abeta,0,0) #this makes it easy to use indexes to identify which ratio is used later
	t=np.identity(len(J))*(1+0.j)
	...
	for k in range(1, len(chainposition)): #for all chain except the main chain construct the upper-triangular part of the transformation matrix
		...
		for i in range(len(link)):  #assing value to superdiagonal with index i (i.e., distance from diagonal), start from row=0 and column=i 
			ax=abeta[link[i]] #The solution abeta has alpha1, alpha2, .. for one chain, gamma1, gamma2,.. for another chain, etc.
			t[link0[0], link[i]]=ax
			j=1
			while i+j <len(link): #j is the index along the superdiagonal 
				ax=ax*chain[link[i+j]]/chain[link0[j]] #ax=alphan*xm, x2=a1/b1,x3=x2*a2/b2,.., y2=a2/b1, y3=y2*a2/b2,..
				t[link0[j], link[i+j]]=ax
				j=j+1
	...
	#See rule2 in "JordanNormalFormAndTriangularMatrix", section "How to make all vectors orthogonal to one main vectors while mainaining Jordan form invariant  "
	for i in range(1,len(link0)): #assing value to superdiagonal i, start from row=0 and column=i 
		ax=abeta[link0[i]] #beta1, beta2, .. are in the solution abeta of the part in the chain "link0"
		t[link0[0], link0[i]]=ax
		j=1
		while i+j<len(link0): #assigning value to super diagonal i, while j is the index along the superdiagonal 
			ax=ax*chain[link0[i+j]]/chain[link0[j]] #p2=b2/b1, p3=p2*b3/b2,.., q2=b3/b1, q3=q2*b4/b2,..
			t[link0[j], link0[i+j]]=ax
			j=j+1
...
def minimizeu0byordernew(u,J,chainposition, powerindex,u0idx, ypowerorder):
	...
	#For discussion on minimization for any specific order see "Jordan Form Reformulation/Gauge-invariance-in-Jordan-space.one"
	#and "Gauge transform of JNF minimize high power terms Joined Blocks.nb"
	if display=="minimizeu0byordernew": print "in minimizeu0byordernew, J="	, prm(J, len(J),len(J))
	maxchainlenposition, maxchainlen, chain, chainposition=findchainposition(J)
	#Construct the matrix equation for the transformation coefficients abeta
	#if ypowerorder==0, then use orderdot with x power counted only, else y powers are also take into account
	...
	ene0=np.empty(len(mc))*(1.+0.j) #column <e_i|e_0>
	for i in range(1,len(u)):
		order=findordernew(i,chainposition,u,powerindex, ypowerorder,u0idx) #For each i select the power order for the minimization
	#if ypowerorder==0, then use orderdot with x power counted only, else y powers are also take into account
...
def UMexpJ(M, mu,powerindex):
	...
	#print "Check standard Jordan form of m: un.mn=Jn.un"
	un,Jn=standardizeJNF(us,Js,usnormalizer) #un, Jn is in the standard Jordan form, un=usnormalizer.u
	#checkJordan(un,Jn,mn)
	...
	u2,t2=minimizeu0byorder(u1,JlnJM,chainposition, powerindex) #Hence u2=t2.u1 is still the left basis of ln(exp(-j*mu)M):   u2.ln(exp(-j*mu)M)=JlnJM.u2, and the high order terms in u2[0] are minimized
	...
	u3,J3,u3normalizer,chain,A2,A2m=normalize(u2,JlnJM) #according to module "normalize", every row of u3 is normalized, J3 is semiJordan form, u3=A2.u2, J3=A2.JlnJM.A2m
	U,J=standardizeJNF(u3,J3,u3normalizer) #U=u3normalizer.u3=u3normalizer.A2.t2.u1=u3normalizer.A2.t2.v.un, "standardizeJNF" chang semiJordan form J3 into standard J
...
def UMexpJnew(M, mu,powerindex,nres,u0idx,ypowerorder):
	...
	un,Jn=standardizeJNF(us,Js,usnormalizer) #un, Jn is in the standard Jordan form, un=usnormalizer.u
	#checkJordan(un,Jn,mn)
	if display=="UMexpJnew": print "in UMexpJnew, Jn=",Jn
	if display=="UMexpJnew": print "in UMexpJnew, len(Jn)=",len(Jn)
	if len(Jn)==1:#when len(Jn)==1, the eigenspace is just the eigenvector, and lnJM is ill defined, so we directly give the simple Jordan form solution which is [0]
		u3,J3,u3normalizer,chain,A2,A2m=normalize(un,Jn)#according to module "normalize", every row of u3 is normalized, J3 is semiJordan form, u3=A2.u2, J3=A2.JlnJM.A2m
		if display=="UMexpJnew": print "in UMexpJnew,2, len(Jn)=",len(Jn)
		U,J=standardizeJNF(u3,J3,u3normalizer) #U=u3normalizer.u3=u3normalizer.A2.t2.u1=u3normalizer.A2.t2.v.un, "standardizeJNF" chang semiJordan form J3 into standard J
	...
	u1=np.dot(v,un) #u1=v.un is the left basis of ln(exp(-j*mu)M):   u1.ln(exp(-j*mu)M)=JlnJM.u1   (because JlnJM=v.un.ln(exp(-j*mu)M).unm.vm, so JlnJM.v.un=v.un.ln(exp(-j*mu)M)   )
	maxchainlenposition, maxchainlen, chain, chainposition=findchainposition(JlnJM)
	u2,t2=minimizeu0byordernew(u1,JlnJM,chainposition, powerindex,u0idx,ypowerorder) #Hence u2=t2.u1 is still the left basis of ln(exp(-j*mu)M):   u2.ln(exp(-j*mu)M)=JlnJM.u2, and the high order terms in u2[0] are minimized
...
def scaling(M,powerindex):
	...
	scalex=(absM.max())**(-(1./(power-1.)))
	scalem=1/scalex
	mlen=len(powerindex)
	if display=="scaling": print "scalex=", scalex, " scalem=", scalem
	As=np.identity(mlen)
	for i in range(mlen):
...
def scalingnew(Ms1,powerindex,scalemf):
	...
	i,j=np.unravel_index(absM.argmax(), absM.shape)
	power=sum(powerindex[j])
	scalex2=(absM.max())**(-(1./(power-1.)))
	scalem2=1/scalex2
	mlen=len(powerindex)
	if display=="scalingnew": print "scalex2=", scalex2, " scalem1=", scalem2
	...
	for i in range(mlen):
        ...
        ms=d3(As2,Ms1,Asm2) #ms does not take the first scaling into account
	scalex=scalex2*scalemf  #scalex now takes the first scaling "scalemf" into account, see line 164 of jf24.py
        return ms,As2,Asm2,scalex

def UMexpJScaling(M, mu,powerindex):
	#This module calculate u such that:
	...
	u,J,vn,un=UMexpJ(Ms, mu,powerindex)
...
def UMsUbarexpJ(Ms1,phix0,nres,powerindex,scalemf,u0idx, ypowerorder):
	Ms,As2,Asm2,scalex=scalingnew(Ms1,powerindex,scalemf) #We no longer use Ms2 as in jf24.py, scalex now include both initial scaling scalemf, and the scaling generated by second scaling 
	u,J,vn,un=UMexpJnew(Ms,phix0,powerindex,nres,u0idx,ypowerorder)#Temporarily we still use all the power including y

	mn=Ms-np.identity(len(Ms))*np.exp(1j*nres*phix0)
	tm1,g=righteigensubspace(mn)
	A=np.dot(u,g)
	Am=np.linalg.inv(A)
	ubar=np.dot(g,Am)
	...
	tmp1=[ i[-1] for i in tmp] #keep only absolute values of u[0] for those terms with only x powers
	tmp2=np.argmax(tmp1) #find the position of the maximum in the list of x power only terms of u[0]
	dmt=tmp[tmp2][0] #find the index in u[0] for the maximum term
	#Hence the normalization of this term is:
	unormalizer=u[0][dmt]
	u=u/unormalizer#this normalizes u matrix so to first order w is same as zsbar=zbar/scalex
