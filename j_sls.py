...
chsext = [sls.ring.getElements('sext','SD')[0],sls.ring.getElements('sext','SFH')[0]]
sls.ring.getChrm(chsext)
...
L = [sls.ring.bl[k].L for k in sls.ring.klist]

tol = 1e-12
betax0,betay0 = sls.ring.betax[0],sls.ring.betay[0]
alphax0,alphay0 = sls.ring.alfax[0],sls.ring.alfay[0]
phix0,phiy0 = sls.ring.nux*np.pi*2,sls.ring.nuy*np.pi*2
...
x0list = [1.0e-3, 4.0e-3, 6.0e-3]
y0list = [4.0e-3, 2.0e-3, 1.0e-3]
xylist = []
...
def cirDist(k2s):
	...
	norder = 7
        ...
        # slow but general ---
	sext = sls.ring.getElements('sext','SXX_MH')[0]
        sext.put('K2',k2s[0])
	sext = sls.ring.getElements('sext','SXY_MH')[0]
        sext.put('K2',k2s[1])
        sext = sls.ring.getElements('sext','SYY_MH')[0]
        sext.put('K2',k2s[2])
        sext = sls.ring.getElements('sext','SDX')[0]
        sext.put('K2',k2s[3])
        sext = sls.ring.getElements('sext','SFXH')[0]
        sext.put('K2',k2s[4])

        sls.ring.cchrom1(chsext,[0.04,0.])

        K2 = [sls.ring.bl[k].K2 for k in sls.ring.klist]
        
        temp = [CTPS(0,i) for i in xrange(1,nv+1)]
        for i,k in enumerate(sls.ring.klist):
            temp = sm.lmPass(sls.ring.mlist[i][:nv,:nv],temp)
            temp = sm.thickSextPass(L[i],K2[i],1,temp)
        temp = sm.lmPass(sls.ring.mlist[-1][:nv,:nv],temp)
	...
	xlist = np.linspace(1e-6, 6e-3,10) # a list of x
	ylist = np.linspace(4e-6, 1e-3,10) # a list of y
