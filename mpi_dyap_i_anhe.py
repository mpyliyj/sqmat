...
def f_anhe(fin):
    ...
    sext.put('K2',fin[0]+fin[6])
    sext = anhe.ring.getElements('sext','sh3w')[0]
    sext.put('K2',fin[1]+fin[7])
    sext = anhe.ring.getElements('sext','sh4w')[0]
    sext.put('K2',fin[2]+fin[8])
    anhe.ring.finddyapsym4(xmin=-0.03,xmax=.03,ymin=1e-6,ymax=.0065,nx=81,ny=16,nturn=256,verbose=0,dfu=0)
...
def mpi_run():
    ...
    if rank == 0:
        fid = h5py.File('./data/anhe_20170204_dyap.h5','a')
